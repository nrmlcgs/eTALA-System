...
if (!isset($_SESSION['verify_token']) || (trim($_SESSION['verify_token']) == '')) {
    ...
    if (isset($_GET['code'])) {
        ...
        $result = $conn->query("SELECT * FROM tbl_student_info WHERE verify_token = '$id'");
    ...
    if (isset($_POST['change-password'])) {
        ...
        if ($password == $c_password) {
            $stmt = $conn->query("UPDATE tbl_student_info SET password = '$password', verify_token = '0' WHERE email = '$id'");
            if ($stmt) {
