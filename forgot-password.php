...
if (isset($_POST['send-email'])) {
    ...
    $check_email_run = $conn->query("SELECT * FROM `tbl_student_info` WHERE email = '$email'");
    if ($check_email_run->num_rows > 0) {
        $row = $check_email_run->fetch_array();
        $getEmail = $row['email'];

        $result = $conn->query("UPDATE tbl_student_info SET verify_token = '$token' WHERE email = '$email'");
        if ($result) {
                    ...
                    <div class="card-body">
                        <form action="forgot-password.php" method="POST">
                            ...
                            <div class="form-group text-center">
                                <a href="index.php" class="text-decoration-none">Back to
                                    login</a>
                            </div>
